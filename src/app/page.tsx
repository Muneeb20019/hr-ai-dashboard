...
export default function PowerRecruitAIHub() {
      ...
      <div className="max-w-[1600px] mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-10 border-b border-slate-800 pb-12">
            <div className="space-y-6">
                <div className="bg-slate-900/90 p-3 rounded-2xl border border-slate-800 shadow-2xl inline-flex items-center gap-4">
                    ...
                    <Text className="text-xs text-slate-500 px-4 font-mono uppercase tracking-[0.2em] font-black border border-slate-800 rounded-lg py-1">SECURE_FILTER</Text>
                </div>
                <div>
                    {/* --- MASSIVE BOLD TAGLINE --- */}
                    <Text className="text-indigo-400 font-black tracking-tight text-4xl mb-4 uppercase italic leading-none drop-shadow-[0_2px_15px_rgba(99,102,241,0.5)]">
                        "AI-Driven Talent Insights for Smarter Decisions"
                    </Text>
                    <Title className="text-9xl font-black text-white tracking-tighter uppercase italic leading-none drop-shadow-2xl">
                        RECRUIT.AI <span className="text-indigo-600 font-thin not-italic lowercase">hub</span>
                    </Title>
                </div>
            </div>
            <Badge color="indigo" className="animate-pulse border-indigo-500/30 px-12 py-5 font-black tracking-[0.3em] text-sm rounded-full bg-indigo-500/10 shadow-[0_0_60px_rgba(99,102,241,0.3)]">
                AI AGENT LIVE
            </Badge>
        </div>

        {/* METRICS GRID - PREVIOUS LARGE SIZE */}
        <Grid numItemsLg={2} className="gap-12 mb-16">
            <Card className="bg-slate-900/40 border-slate-800/60 ring-0 backdrop-blur-xl p-12 rounded-[3rem]">
                ...
                <Metric className="text-white font-black text-[8rem] leading-none tracking-tighter">{allCandidates.length}</Metric>
            ...
            <Card className="bg-slate-900/40 border-slate-800/60 ring-0 flex flex-col justify-center items-center py-16 text-center rounded-[3rem] border-l-8 border-l-indigo-500 shadow-2xl">
                ...
                <Metric className="text-white font-black text-[14rem] leading-none drop-shadow-[0_0_60px_rgba(99,102,241,0.5)]">
                    {filteredCandidates.length}
                </Metric>
            </Card>
        </Grid>

        {/* DATA TABLE - PREVIOUS LARGE SIZE */}
        <Card className="bg-slate-900/20 border-slate-800/40 ring-0 rounded-[4rem] overflow-hidden shadow-3xl">
          <div className="p-12 border-b border-slate-800/60 bg-slate-900/40 flex justify-between items-center">
            <Title className="text-white font-black tracking-tighter uppercase text-5xl italic leading-none">Intelligence Stream — {selectedDate?.toLocaleDateString()}</Title>
          </div>
          <Table>
              ...
              {filteredCandidates.map((item) => (
                <TableRow key={item.id} className="hover:bg-indigo-500/5 transition-all group border-b border-slate-800/30">
                  <TableCell className="p-10">
                    <Text className="text-white font-black text-5xl uppercase leading-none mb-3 tracking-tighter group-hover:text-indigo-400 transition-colors uppercase italic leading-none">{item.name}</Text>
                    <Text className="text-slate-600 text-[10px] font-mono italic uppercase tracking-widest leading-none">{new Date(item.created_at).toLocaleTimeString()}</Text>
                  </TableCell>
                  <TableCell className="p-10 text-center">
                    <div className="w-64 mx-auto">
                        <Text className="text-indigo-400 font-black text-sm mb-3 italic text-center uppercase tracking-widest">{item.score}/10 MATCH RANK</Text>
                  ...
                  <TableCell className="p-10 text-right">
                    <Button size="xl" variant="secondary" className="bg-indigo-600/10 text-indigo-400 border-indigo-500/40 hover:bg-indigo-600 hover:text-white transition-all font-black px-16 py-8 rounded-[2rem] uppercase text-lg tracking-widest shadow-2xl shadow-indigo-500/10" onClick={() => { setSelectedCandidate(item); setIsOpen(true); }}>
                        PROFILE <ArrowRight className="ml-5 w-8 h-8" />
                    </Button>
                  </TableCell>
                </TableRow>
              ))}
          ...
          {!hasData && <div className="text-center py-60 bg-slate-950/40"><Text className="text-slate-700 font-mono italic text-lg tracking-[0.5em] animate-pulse uppercase font-black">AWAITING NEURAL DATA PACKETS...</Text></div>}
        </Card>
      </div>

      {/* --- RE-SIZED MODAL WITH CLEAR DETAILS --- */}
      <Dialog open={isOpen} onClose={() => setIsOpen(false)} static={true}>
        <DialogPanel className="bg-[#020617] border border-slate-800 p-0 rounded-[4rem] max-w-6xl shadow-[0_0_200px_rgba(79,70,229,0.3)] overflow-hidden">
          
          <div className="bg-slate-900/60 p-12 border-b border-slate-800 flex justify-between items-center">
            <div>
                <Badge color="emerald" className="px-6 py-2 font-black text-xs rounded-full uppercase tracking-widest mb-3">AI EVALUATED</Badge>
          ...
          <div className="p-12 space-y-12">
            <div className="grid grid-cols-4 gap-8">
                {[
                    ...
                    <div key={i} className="bg-slate-900/50 border border-slate-800 p-8 rounded-[2.5rem] text-center shadow-2xl">
                        <Text className="text-slate-600 font-black uppercase text-[10px] tracking-[0.3em] mb-2 leading-none">{m.label}</Text>
                        <Text className={`${m.col} font-black text-5xl tracking-tighter leading-none`}>{m.val}</Text>
                    </div>
                ))}
            </div>
            ...
            <Grid numItemsMd={2} className="gap-12">
                <div className="space-y-10">
                    <div className="p-10 bg-slate-900/40 rounded-[3rem] border border-slate-800 shadow-inner">
                        <Title className="text-white text-[10px] uppercase tracking-[0.4em] font-black mb-10 border-b border-slate-800 pb-4 flex items-center italic tracking-widest"><Phone className="w-6 h-6 mr-4 text-indigo-500" /> Secure Communications</Title>
                        <div className="space-y-10">
                            <div><Text className="text-slate-600 text-[10px] font-black uppercase tracking-widest mb-2">Authenticated Email Address</Text><Text className="text-indigo-400 text-3xl font-black truncate tracking-tighter leading-none underline decoration-indigo-500/20 underline-offset-8">{selectedCandidate?.email}</Text></div>
                ...
                <div className="p-12 bg-indigo-600/5 border border-indigo-500/10 rounded-[3rem] shadow-2xl relative overflow-hidden flex flex-col justify-start max-h-[450px]">
                    <Title className="text-indigo-400 text-[10px] uppercase font-black mb-8 italic underline underline-offset-[16px] decoration-indigo-500/30 tracking-[0.5em]">AI Analysis</Title>
            ...
            <div className="flex gap-10 mt-16 pb-8">
                <Button className="w-full py-12 rounded-[3rem] bg-indigo-600 hover:bg-indigo-500 font-black text-3xl border-none shadow-[0_20px_80px_rgba(79,70,229,0.5)] hover:-translate-y-2 transition-all uppercase tracking-[0.2em] italic active:scale-95">INITIATE INTERVIEW</Button>
                <Button variant="secondary" className="w-1/4 py-12 rounded-[3rem] border-slate-800 text-slate-500 font-black hover:bg-rose-500/10 hover:text-rose-400 transition-all uppercase tracking-widest">ARCHIVE</Button>
            </div>
          </div>
        </DialogPanel>
      </Dialog>
